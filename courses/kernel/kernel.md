...
[点击这里从百度网盘下载对应平台的虚拟机镜像](https://chenxiaosong.com/baidunetdisk)，`x86_64`（也就是你平时用来安装windows系统的电脑，或者比较老的苹果电脑）选择`ubuntu-amd64.zip`，`arm64`（新的苹果电脑）选择`ubuntu-arm64.zip`。虚拟机运行后，登录界面的密码是`1`。
