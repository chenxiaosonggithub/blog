...
[点击这里从百度网盘下载配套的视频教程](https://pan.baidu.com/s/12hDXdKdIE4-ggjcS7-88lw?pwd=1234)。
...
[点击这里从百度网盘下载对应平台的虚拟机镜像](https://pan.baidu.com/s/12hDXdKdIE4-ggjcS7-88lw?pwd=1234)，`x86_64`（也就是你平时用来安装windows系统的电脑，或者比较老的苹果电脑）选择`ubuntu-amd64.zip`，`arm64`（新的苹果电脑）选择`ubuntu-arm64.zip`。虚拟机运行后，登录界面的密码是`1`。
...
sudo apt-get install virt-manager -y # 虚拟机图形界面，可能需要重启才能以非root用户启动virt-manager，当然对于内核开发来说安装这个软件是为了生成自动生成virbr0网络接口
...
make O=build Image ${KNLMKFLGS} # aarch64，比如比较新的arm芯片的苹果电脑上vmware fusion安装的ubuntu
...
制作好的Ubuntu虚拟机镜像（从百度网盘中下载的）中的`/home/sonvhi/qemu-kernel/start.sh`脚本中每个选项的可选值可以使用以下命令查看：
```sh
...
完整的启动命令查看制作好的Ubuntu虚拟机镜像（从百度网盘中下载的）中的`/home/sonvhi/qemu-kernel/start.sh`脚本。
...
各位朋友可以[点击这里从百度网盘下载pdf电子书](https://pan.baidu.com/s/12hDXdKdIE4-ggjcS7-88lw?pwd=1234)。电子书请仅作为学习用途，有需要的话建议购买纸质书。
