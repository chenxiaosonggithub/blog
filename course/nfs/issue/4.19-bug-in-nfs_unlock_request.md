...
int main(void)
{
    ...
    // 指定要使缓存失效的区域（例如，从偏移 0x1000 开始，长度 0x1000 字节）
    off_t offset = 0;
    off_t len = 4096;

    // POSIX_FADV_DONTNEED：提示内核丢弃该区域的页缓存
    int ret = posix_fadvise(fd, offset, len, POSIX_FADV_DONTNEED);
    if (ret != 0) {
        errno = ret;
