...
- 弹出配置界面，`概况 固件: UEFI aarch64 -> cpu数 型号: cortex-a72`
- `添加硬件 图形 类型: spice服务器 地址: 所有接口 -> 添加硬件 输入 USB鼠标 USB键盘`。
- `添加硬件 图形 类型:`建议选择`spice服务器`（如果选`vnc服务器`，要把virt-manager窗口关闭才能用vnc客户端登录，而且系统鼠标定位有一点小问题，所以安装阶段不建议选择`vnc服务器`）
