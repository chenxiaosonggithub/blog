...
[点击这里访问代码仓库](https://github.com/chenxiaosonggithub/myfs)。
...
修改[`Makefile`](https://github.com/chenxiaosonggithub/myfs/blob/master/Makefile)文件中的`KDIR`变量对应Linux内核仓库的路径，然后在[`myfs`代码仓库](https://github.com/chenxiaosonggithub/myfs)执行以下命令:
```sh
make # 生成 myfs.ko
...
把[整个代码仓库目录`myfs`](https://github.com/chenxiaosonggithub/myfs)复制到Linux内核仓库的`fs`目录下。然后到内核仓库中执行以下命令:
```sh
