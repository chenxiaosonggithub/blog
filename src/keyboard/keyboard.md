...
默认按`caps_lock`键会切换输入法，设置 `Keyboard` -> `Text input` -> `Input Sources` -> `Edit` -> `All Input Sources` -> 关闭`Use the Caps Lock key to switch to and from ABC`

默认按`caps_lock`键会切换输入法，设置 `Keyboard` -> `Press fn(是一个地球图标) key to` 选 `Do Nothing`。

我使用的配置文件: [karabiner.json](https://gitee.com/chenxiaosonggitee/blog/blob/master/src/keyboard/karabiner.json)，放到macOS的`~/.config/karabiner/karabiner.json`路径下。
...
v1版本的配置: [autohotkey-v1.1.ahk](https://gitee.com/chenxiaosonggitee/blog/blob/master/src/keyboard/autohotkey-v1.1.ahk)。
