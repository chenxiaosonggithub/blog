我是自由软件的狂热者，使用的操作系统是Linux，虽然爽但还是有部分商业软件未提供Linux版本，所以难免要使用其他的操作系统。比如我会在Linux的QEMU/KVM虚拟机下安装macOS（安装方法查看[《QEMU/KVM安装macOS系统》](https://chenxiaosong.com/linux/qemu-kvm-install-macos.html)），macOS访问Linux的文件就需要安装samba服务（没错，我就是不用windows系统）。
...
[forVM]
    ...
    #如果无法访问，create mask设置为0770或0777
    create mask = 0700
    #如果无法访问，directory mask设置为0770或0777
    directory mask = 0700
...
> 注意：
>
> 1. 如果无法访问，create mask和directory mask设置为0770或0777
> 2. 配置文件中一般会有默认`[homes]`选项，表示访问账号的家目录，删除`[homes]`选项将无法访问家目录
> 3. 修改完配置后，要重启samba服务：Ubuntu命令为**`sudo systemctl restart smbd.service`**

如果windows和macOS还是无法访问Linux的文件夹，Fedora需要再进行以下步骤：
