  ...
  cifs_demultiplex_thread
    cifs_read_from_socket
      cifs_readv_from_socket
        ...
        cifs_reconnect
          list_move(&mid_entry->qhead, &retry_list) // 在GlobalMid_Lock的保护下将pending_mid_q链表移到retry_list链表
          list_del_init // 从retry_list链表中删除，无任何锁保护
    ...
    smb2_setup_request // ses->server->ops->setup_request
      smb2_get_mid_entry
        *mid = smb2_mid_entry_alloc
        list_add_tail // 加到pending_mid_q链表中，有GlobalMid_Lock保护
    out:
    cifs_delete_mid
      list_del_init // 在GlobalMid_Lock的保护下从链表中删除（可能是pending_mid_q链表也可能是retry_list链表）
```
