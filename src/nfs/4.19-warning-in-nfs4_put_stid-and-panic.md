...
报`WARNING`是在`delete_node`函数的`0x98`偏移量（10进制`152`），`delete_node`函数前面一部分的汇编：
```sh
crash> dis -l delete_node
...
/usr/src/debug/kernel-4.19.90/linux-4.19.90-24.4.v2101.ky10.aarch64/./include/linux/compiler.h: 223
0xffff505f0a38d7a4 <delete_node+140>:   ldr     x0, [x0,#24]
/usr/src/debug/kernel-4.19.90/linux-4.19.90-24.4.v2101.ky10.aarch64/lib/radix-tree.c: 784
0xffff505f0a38d7a8 <delete_node+144>:   cmp     x3, x0
0xffff505f0a38d7ac <delete_node+148>:   b.eq    0xffff505f0a38d8e8
0xffff505f0a38d7b0 <delete_node+152>:   brk     #0x800
```

我们看到panic时无法读的地址`ffffa628c7d2bb28`是报`WARNING`时的`x3`寄存器，报`WARNING`时执行到`list_empty`函数的`return READ_ONCE(head->next) == head`，`include/linux/compiler.h: 223`是执行到`__read_once_size`函数，所以`x0`是`head->next`的值，`x3`是`head`的值，也就是`&node->private_list`的值。
