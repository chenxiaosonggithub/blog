...
x86_64架构下整数参数使用的寄存器依次为：RDI，RSI，RDX，RCX，R8，R9，从栈帧数据中可以看出函数`int nfs_readpage_async(struct nfs_open_context *ctx, struct inode *inode, struct page *page)`第三个参数为`RDX: 01d7ffffc0000001`，显然不是一个有效的地址，说明`page`已经被释放。
